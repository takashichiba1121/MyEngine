...
void SpriteCommon::Initialize(DirectXCommon* DxCommon)
{
	...
	InitializeDissolveGraphicsPipeline();
...
void SpriteCommon::InitializeDissolveGraphicsPipeline()
{
	HRESULT result;
...
//errorBlobから内容をstring型にコピー
	...
	descriptorRange2.BaseShaderRegister = 1;//テクスチャレジスタ0番
	...
	rootParams[ 3 ].DescriptorTable.pDescriptorRanges = &descriptorRange2;//デスクリプタレンジ
	...
	assert(SUCCEEDED(result));
	result = dxCommon->GetDevice()->CreateRootSignature(0,rootSigBlob->GetBufferPointer(),rootSigBlob->GetBufferSize(),
		IID_PPV_ARGS(&dissolveRootSignature));
	assert(SUCCEEDED(result));
	//rootSigBlob->Release();
	//パイプラインにルートシグネチャをセット
	pipelineDesc.pRootSignature = dissolveRootSignature.Get();

	//パイプランスステートの生成
	result = dxCommon->GetDevice()->CreateGraphicsPipelineState(&pipelineDesc,IID_PPV_ARGS(&dissolvePipelineState));
	assert(SUCCEEDED(result));
}
...
void SpriteCommon::DissolvePreDraw()
{
		//パイプラインステートとルートシグネチャの設定コマンド
	dxCommon->GetCommandList()->SetPipelineState(dissolvePipelineState.Get());
	dxCommon->GetCommandList()->SetGraphicsRootSignature(dissolveRootSignature.Get());
}

void SpriteCommon::DissolvePostDraw()
{
}
...
void SpriteCommon::Finalize()
{
	...
	dissolvePipelineState = nullptr;
	dissolveRootSignature = nullptr;
	//sDxCommon = nullptr;
}
